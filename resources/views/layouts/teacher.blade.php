...
<head>
    ...
    <title>@yield('title', 'Teacher - RemedialHub')</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', sans-serif; background: #f1f5f9; min-height: 100vh; }
        .sidebar { width: 260px; background: linear-gradient(180deg, #0f172a 0%, #1e293b 100%); min-height: 100vh; position: fixed; left: 0; top: 0; z-index: 40; }
        .sidebar-link { display: flex; align-items: center; padding: 12px 20px; color: #94a3b8; text-decoration: none; font-size: 14px; transition: all 0.2s; border-left: 3px solid transparent; }
        .sidebar-link:hover, .sidebar-link.active { background: rgba(99,102,241,0.15); color: #fff; border-left-color: #6366f1; }
            ...
            <a href="{{ route('teacher.dashboard') }}" style="text-decoration: none;">
                <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <div>
                        ...
                        <div style="color: #818cf8; font-size: 11px;">Teacher Portal</div>
                    </div>
                </div>
            </a>
        </div>
        <nav style="padding: 12px 0;">
            <a href="{{ route('teacher.dashboard') }}" class="sidebar-link {{ request()->routeIs('teacher.dashboard') ? 'active' : '' }}"><i class="fas fa-th-large"></i> Dashboard</a>
            <a href="{{ route('teacher.students') }}" class="sidebar-link {{ request()->routeIs('teacher.students') ? 'active' : '' }}"><i class="fas fa-users"></i> My Students</a>
            <a href="{{ route('teacher.courses') }}" class="sidebar-link {{ request()->routeIs('teacher.courses*') ? 'active' : '' }}"><i class="fas fa-book-open"></i> Courses</a>
            <a href="{{ route('teacher.videos') }}" class="sidebar-link {{ request()->routeIs('teacher.videos*') ? 'active' : '' }}"><i class="fas fa-play-circle"></i> Videos</a>
            <a href="{{ route('teacher.articles') }}" class="sidebar-link {{ request()->routeIs('teacher.articles*') ? 'active' : '' }}"><i class="fas fa-file-alt"></i> Articles/Notes</a>
            <a href="{{ route('teacher.assessments') }}" class="sidebar-link {{ request()->routeIs('teacher.assessments*') ? 'active' : '' }}"><i class="fas fa-clipboard-check"></i> Assessments</a>
            <a href="{{ route('teacher.questions') }}" class="sidebar-link {{ request()->routeIs('teacher.questions*') ? 'active' : '' }}"><i class="fas fa-question-circle"></i> Questions</a>
            <a href="{{ route('teacher.results') }}" class="sidebar-link {{ request()->routeIs('teacher.results') ? 'active' : '' }}"><i class="fas fa-poll"></i> Results</a>
            <a href="{{ route('teacher.slow-learners') }}" class="sidebar-link {{ request()->routeIs('teacher.slow-learners') ? 'active' : '' }}"><i class="fas fa-exclamation-triangle"></i> Slow Learners</a>
            <a href="{{ route('teacher.remedial-classes') }}" class="sidebar-link {{ request()->routeIs('teacher.remedial-classes*') ? 'active' : '' }}"><i class="fas fa-video"></i> Remedial Classes</a>
            <a href="{{ route('teacher.assignments') }}" class="sidebar-link {{ request()->routeIs('teacher.assignments*') ? 'active' : '' }}"><i class="fas fa-tasks"></i> Assignments</a>
            <a href="{{ route('teacher.submissions') }}" class="sidebar-link {{ request()->routeIs('teacher.submissions*') ? 'active' : '' }}"><i class="fas fa-inbox"></i> Submissions</a>
            <a href="{{ route('teacher.progress') }}" class="sidebar-link {{ request()->routeIs('teacher.progress*') ? 'active' : '' }}"><i class="fas fa-chart-line"></i> Progress Reports</a>
            ...
            <div style="display: flex; align-items: center; gap: 16px;">
                <div style="display: flex; align-items: center; gap: 10px;">
                    <div style="width: 36px; height: 36px; background: #ede9fe; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                        <i class="fas fa-chalkboard-teacher" style="color: #6366f1; font-size: 14px;"></i>
                    </div>
                    <div>
